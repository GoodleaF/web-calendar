...
<% 
java.util.Calendar cal=java.util.Calendar.getInstance();				//실제 시간을 계산하기 위한 부분
...
int day1=cal.get(java.util.Calendar.DAY_OF_WEEK);									//현재 요일을 가져온다.(일요일은 1 토요일은 7)
int last=cal.getActualMaximum(java.util.Calendar.DAY_OF_MONTH);						//현재 월의 마지막 일을 가져온다.
int br=0;
for(int i=0; i<day1-1;i++){
	out.println("<td>&nbsp;</td>");
	br++;
	if((br)%7==0){
		out.println("<br>");
...
for(int i=1; i<=last;i++){
	if((br - 1) == thisday){
		out.println("<td bgcolor=#ffff00>"+i+"</td>");
	}
	else{
		out.println("<td>"+i+"</td>");
	}
